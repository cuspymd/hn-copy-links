...
- Confirm the target version (for example `0.2.0`).
...
- Confirm `npm test` and `npm run test:e2e` pass.
...
- `outputs/hn-copy-links-<version>-chrome.zip`
- `outputs/hn-copy-links-<version>-firefox.zip`
...
- Group items by categories (UI/UX, reliability, architecture/tests).
...
No automated test covers it. Before a release that changed the button, the
styles or the clipboard path, check on a device by hand and say in the report
